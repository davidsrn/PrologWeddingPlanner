...
score_all(_, [], []).
score_all(Host, [P|T], [(P, S)|Rest]) :-
    score(Host, P, S),
    score_all(Host, T, Rest).
...
rem_extras(0, Rest, [], Rest) :- !.
rem_extras(_, [], [], []) :- !.
rem_extras(N, [H|T], [H|Taken], Missing) :-
    N1 is N - 1,
    rem_extras(N1, T, Taken, Missing).

rem_tuple([], New, New).
rem_tuple([(Val, _)|T], New, Res) :-
    rem_tuple(T, [Val|New], Res).

sum_scores([], 0).
sum_scores([(_, S)|T], Total) :-
    sum_scores(T, Rest),
    Total is Rest + S.

candidate(Seats, Persons, Total-(Host-Guests)) :-
    select(Host, Persons, Others),
    score_all(Host, Others, Scored),
    pair_sort(Scored, Sorted),
    rem_extras(Seats, Sorted, Guests, _),
    sum_scores(Guests, Total).

format_out(Host, []) :-
    !,
    format("~w will be seated alone since no guests remain.~n", [Host]).
format_out(Host, Guests) :-
    format("~w will be seated with ~w since their relationship score is the highest.~n", [Host, Guests]).
...
main_h(_, [], Acc, Res) :-
    reverse(Acc, Res).
main_h(Tables, Persons, Acc, Res) :-
    Persons \= [],
    Seats is Tables - 1,
    findall(C, candidate(Seats, Persons, C), Cs),
    max_member(_-(Host-Guests), Cs),
    format_out(Host, Guests),
    rem_tuple(Guests, [], GuestNames),
    subtract(Persons, [Host|GuestNames], Remaining),
    main_h(Tables, Remaining, [Host-Guests|Acc], Res).
