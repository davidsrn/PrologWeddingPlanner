...
person_units([], []).
person_units([P|T], [[P, Q]|Rest]) :-
    partner(P, Q),
    select(Q, T, T2),
    !,
    person_units(T2, Rest).
person_units([P|T], [[P]|Rest]) :-
    person_units(T, Rest).

unit_size(U, S) :- length(U, S).

pack_units(Units, Size, NT, Tables) :-
    predsort([O, A, B]>>(unit_size(A, LA), unit_size(B, LB),
                         ( LA > LB -> O = (<) ; O = (>) )),
             Units, Sorted),
    length(Bins, NT),
    maplist(=(0-[]), Bins),
    foldl(place_unit(Size), Sorted, Bins, Packed),
    findall(Us, member(_-Us, Packed), Tables).

place_unit(Size, U, Bins0, Bins) :-
    unit_size(U, SU),
    (   ff_place(U, SU, Size, Bins0, Bins) -> true
    ;   throw(error(packing_failed(U), _))
    ).

ff_place(U, SU, Size, [Occ-Us|Rest], [Occ1-[U|Us]|Rest]) :-
    Occ1 is Occ + SU,
    Occ1 =< Size,
    !.
ff_place(U, SU, Size, [B|Rest], [B|Rest1]) :-
    ff_place(U, SU, Size, Rest, Rest1).

init_state(Tables, TA, PA) :-
    empty_assoc(E),
    foldl([Units, Id0-T0-P0, Id1-T1-P1]>>(
        Id1 is Id0 + 1,
        put_assoc(Id0, T0, Units, T1),
        foldl([U, Pi, Po]>>put_assoc(U, Pi, Id0, Po), Units, P0, P1)
    ), Tables, 1-E-E, Final),
    Final = _-TA-PA.

table_members(Units, Members) :-
    foldl([U, A0, A1]>>append(U, A0, A1), Units, [], Members).

occupancy(Units, N) :-
    table_members(Units, M),
    length(M, N).

total(TA, Total) :-
    assoc_to_values(TA, Tables),
    maplist([Us, S]>>(table_members(Us, M), table_score(M, S)), Tables, Ss),
    sum_list(Ss, Total).
...
step(UArr, NU, NT, Size, Temp, TA0-PA0-Tot0, TA-PA-Tot) :-
    random_between(1, NU, I),
    arg(I, UArr, U),
    get_assoc(U, PA0, T1),
    random_between(1, NT, T2),
    T2 =\= T1,
    get_assoc(T1, TA0, Units1),
    get_assoc(T2, TA0, Units2),
    selectchk(U, Units1, U1Rest),
    table_members(U1Rest, M1R),
    table_members(Units2, M2),
    unit_size(U, SU),
    occupancy(Units2, Occ2),
    Free2 is Size - Occ2,
    (   Free2 >= SU, random(R), R < 0.5
    ->  uscore(U, M2, Gain),
        uscore(U, M1R, Loss),
        Delta is Gain - Loss,
        accept(Delta, Temp),
        put_assoc(T1, TA0, U1Rest, TA1),
        put_assoc(T2, TA1, [U|Units2], TA),
        put_assoc(U, PA0, T2, PA),
        Tot is Tot0 + Delta
    ;   length(Units2, LU2),
        LU2 >= 1,
        random_between(1, LU2, J),
        nth1(J, Units2, V),
        unit_size(V, SV),
        occupancy(U1Rest, Occ1R),
        Occ1R + SV =< Size,
        Occ2 - SV + SU =< Size,
        selectchk(V, Units2, U2Rest),
        table_members(U2Rest, M2R),
        uscore(U, M2R, GA), uscore(V, M1R, GB),
        uscore(U, M1R, LA), uscore(V, M2R, LB),
        Delta is GA + GB - LA - LB,
        accept(Delta, Temp),
        put_assoc(T1, TA0, [V|U1Rest], TA1),
        put_assoc(T2, TA1, [U|U2Rest], TA),
        put_assoc(U, PA0, T2, PA1),
        put_assoc(V, PA1, T1, PA),
        Tot is Tot0 + Delta
    ).

loop(Deadline, Budget, UArr, NU, NT, Size, Iter0, State0, State, Iters) :-
    get_time(Now),
    (   Now >= Deadline
    ->  State = State0, Iters = Iter0
    ;   Elapsed is Now - (Deadline - Budget),
        temp(Elapsed, Budget, Temp),
        run_block(512, UArr, NU, NT, Size, Temp, State0, State1),
        Iter1 is Iter0 + 512,
        loop(Deadline, Budget, UArr, NU, NT, Size, Iter1, State1, State, Iters)
    ).

run_block(0, _, _, _, _, _, S, S) :- !.
run_block(K, UArr, NU, NT, Size, Temp, S0, S) :-
    (   step(UArr, NU, NT, Size, Temp, S0, S1) -> true ; S1 = S0 ),
    K1 is K - 1,
    run_block(K1, UArr, NU, NT, Size, Temp, S1, S).
...
main(Size, Persons, Seconds, Res) :-
    Size >= 2,
    init_edges,
    person_units(Persons, Units),
    length(Persons, N),
    NT is (N + Size - 1) // Size,
    pack_units(Units, Size, NT, Tables0),
    init_state(Tables0, TA0, PA0),
    total(TA0, Seed),
    ...
    loop(Deadline, Seconds, UArr, NU, NT, Size, 0, TA0-PA0-Seed, TA-PA-Total, Iters),
    format("annealing: ~w iterations in ~w s~n", [Iters, Seconds]),
    assoc_to_values(TA, TableUnits0),
    exclude(==([]), TableUnits0, TableUnits),
    maplist(table_members, TableUnits, Members),
    report(Members, 1, Res),
    split_couples(PA, SC),
