...
capacity(Vars, N, Size) :-
    numlist(1, N, Keys),
    length(Counts, N),
    Counts ins 0..Size,
    pairs_keys_values(GC, Keys, Counts),
...
sym_break_([], _).
sym_break_([V|Vs], MaxSoFar) :-
    V #=< MaxSoFar + 1,
    M #= max(MaxSoFar, V),
    sym_break_(Vs, M).

contributions([], []).
contributions([PV|Rest], Terms) :-
    contributions_one(PV, Rest, T1),
    contributions(Rest, T2),
    append(T1, T2, Terms).

contributions_one(_, [], []).
contributions_one(P-V, [Q-W|Rest], Terms) :-
    score(P, Q, S),
    contributions_one(P-V, Rest, More),
    (   S > 0
    ->  B #<==> (V #= W),
        C #= B * S,
        Terms = [C|More]
...
group_tables(PVs, Tables) :-
    pairs_values(PVs, Vs),
    sort(Vs, TableNums),
    findall(Members,
            ( member(T, TableNums),
              findall(P, member(P-T, PVs), Members)
            ),
            Tables).

row_score(_, [], 0).
row_score(P, [Q|Rest], S) :-
    score(P, Q, S1),
    row_score(P, Rest, S2),
    S is S1 + S2.

table_score([], 0).
table_score([P|Rest], S) :-
    row_score(P, Rest, S1),
    table_score(Rest, S2),
    S is S1 + S2.
...
main(TableSize, Persons, Res) :-
    TableSize >= 1,
    length(Persons, N),
    length(Vars, N),
    Vars ins 1..N,
    pairs_keys_values(PVs, Persons, Vars),
    capacity(Vars, N, TableSize),
    sym_break(Vars),
    contributions(PVs, Cs),
    sum(Cs, #=, Total),
    labeling([max(Total), ff], Vars),
    !,
    group_tables(PVs, Tables),
    report(Tables, 1, Res),
